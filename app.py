...
if not st.session_state.logado:
    ...
    with col_btn2:
        if st.button("📝 Cadastro", key="btn_cadastro", use_container_width=True):
            st.session_state.tab_ativa = 1
            st.rerun()
    
    with col_btn3:
        if st.button("🔑 Recuperação", key="btn_recuperacao", use_container_width=True):
            st.session_state.tab_ativa = 2
            st.rerun()
            
    st.divider()
 
       # ===== ABA 0: LOGIN =====
    if st.session_state.tab_ativa == 0:
        ...
        with st.form("form_login"):
            ...
            col_submit, col_cadastrar = st.columns(2)
            with col_submit:
            ...
            if submit_login:
                if not u_log or not s_log:
                    ...
                    if df_u.empty:
                        ...
                        user_row = df_u[df_u["login"] == u_log]
                        if not user_row.empty:
                            hash_armazenado = str(user_row.iloc[0]["senha_hash"])
   ...
   # ===== ABA 1: CADASTRO =====
    elif st.session_state.tab_ativa == 1:
        ...
        with st.form("form_registro"):
            ...
            col_submit, col_voltar = st.columns(2)
            with col_submit:
                submit_registro = st.form_submit_button("✅ Cadastrar", use_container_width=True)
            with col_voltar:
                submit_voltar = st.form_submit_button("⬅️ Voltar", use_container_width=True)
            
            if submit_registro:
                # Validações
                if not n_reg or not u_reg or not s_reg or not s_conf or not c_reg:
                    ...
                    if n_reg in df_u["nome_completo"].values:
                        ...
                        st.info("Voltando para login em 2 segundos...")
                        st.session_state.tab_ativa = 0
                        time.sleep(2)
                        st.rerun()
  ...
  # ===== ABA 2: RECUPERAÇÃO DE SENHA =====
    elif st.session_state.tab_ativa == 2:
        ...
        with st.form("form_recuperacao"):
            ...
            col_submit, col_voltar = st.columns(2)
            with col_submit:
                submit_atualizar = st.form_submit_button("🔄 Atualizar Senha", use_container_width=True)
            with col_voltar:
                submit_voltar = st.form_submit_button("⬅️ Voltar", use_container_width=True)
